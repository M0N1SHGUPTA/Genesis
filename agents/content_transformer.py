...
_MAX_TOKENS = 3500  # full blueprint with 10-15 slides needs ~2.5k tokens
...
1. Every bullet, focus, description, and stat MUST come from the insights/data in the slide contexts above
2. NEVER repeat the document subtitle or cover text on any content slide
3. NEVER use vague filler like "see document", "various factors", "strategic objectives", or "key aspects"
4. Each slide must have UNIQUE content — the same sentence must not appear on two different slides
5. Bullet points: max 15 words each, must contain a specific name, number, date, or company
6. Cover subtitle: max 20 words, one sentence
7. Chart values MUST be plain numbers (not strings): [10, 25, 47] not ["10", "25", "47"]
8. three_cards MUST always have exactly 3 cards with distinct headings
9. key_stats values must be actual figures from the document (e.g. "$69.7B", "80+", "35%")
10. conclusion points must synthesise real findings, not restate section headings
"""
    ...
    def _build_slide(
        ...
        if slide_type == "executive_summary":
            ...
            return {
                ...
                "left": {"heading": "Key Findings", "points": bullets[:mid] or ["See document."]},
                "right": {"heading": "Implications", "points": bullets[mid : mid + 4] or bullets[:4] or ["See document."]},
            }
        ...
        if slide_type == "agenda":
            return {
                ...
                "type": "agenda",
                "title": "Agenda",
                "points": [s["heading"] for s in extracted.get("key_sections", [])][:12] or ["Overview"],
            }
        ...
        if slide_type == "chart":
            ...
            if not chart_data:
                chart_data = {"categories": ["A", "B", "C"], "series": [{"name": "Value", "values": [1, 2, 3]}]}
            return {
                "slide_number": num,
                "type": "chart",
            ...
            cards = [
                {
                    ...
                    "heading": _first_words(ins, 5),
                    "points": [ins],
                }
                for i, ins in enumerate((insights[:3] or ["Point 1", "Point 2", "Point 3"]))
            ]
            # Always pad to exactly 3 cards
            while len(cards) < 3:
                cards.append({"number": str(len(cards) + 1).zfill(2), "heading": "Point", "points": ["See document."]})
            return {"slide_number": num, "type": "content", "layout": "three_cards", "title": title, "cards": cards[:3]}

        if layout in ("two_column", "comparison"):
            mid = max(1, len(insights) // 2)
            left_pts = insights[:mid] or ["See document."]
            right_pts = insights[mid : mid + 4] or insights[:4] or ["See document."]
            comp = sec.get("comparison")
            if comp:
                left_pts = comp.get("left_points", left_pts)
                right_pts = comp.get("right_points", right_pts)
            return {
                "slide_number": num,
                "type": "content",
                "layout": layout,
                "title": title,
                "left": {"heading": comp.get("left_heading", "Overview") if sec.get("comparison") else "Overview", "points": left_pts[:6]},
                "right": {"heading": comp.get("right_heading", "Details") if sec.get("comparison") else "Details", "points": right_pts[:6]},
            }

        if layout == "key_stats":
            stats = extracted.get("global_stats", [])
            if not stats:
                stats = [{"value": "—", "label": "Key metric"}]
            return {"slide_number": num, "type": "content", "layout": "key_stats", "title": "Key Statistics", "stats": stats[:4]}

        if layout in ("timeline", "process_flow"):
            steps = sec.get("process_steps") or [
                {"number": str(i + 1).zfill(2), "heading": _first_words(ins, 5), "description": ins}
                for i, ins in enumerate(insights[:5])
            ] or [{"number": "01", "heading": title, "description": "See document."}]
            return {"slide_number": num, "type": "content", "layout": layout, "title": title, "steps": steps[:5]}
        ...
        if layout == "icon_list":
            items = [
                ...
                for i, ins in enumerate((insights[:4] or ["Key point"]))
            ]
            if not items:
                items = [{"number": "01", "heading": "Overview", "description": "See document."}]
            return {"slide_number": num, "type": "content", "layout": "icon_list", "title": title, "items": items[:4]}

        # single_focus (default for conclusion and catch-all)
        focus = insights[0] if insights else title
        return {
            "slide_number": num,
            "type": slide_type if slide_type in ("conclusion", "content") else "content",
            "layout": "single_focus",
            "title": "Key Takeaways" if slide_type == "conclusion" else title,
            "focus": _first_words(focus, 20),
            "points": [_first_words(p, 15) for p in insights[:6]],
        }
